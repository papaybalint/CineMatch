...
export default function Movies() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/movies/new-releases?page=${page}&limit=18`)
      .then((res) => res.json())
      .then((data) => {
        if (page === 1) {
          setMovies(data)
  ...
  return (
    <section className="content-section">
      ...
      {loading && <div style={{ color: 'white' }}>Új filmek töltése...</div>}
      {error && <div style={{ color: 'red' }}>Hiba történt a filmek töltéskor.</div>}
